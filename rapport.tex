...
\begin{IEEEeqnarray*}{rCl}
KA&=&\frac{\omega}{\omega_o}A
\end{IEEEeqnarray*}
Där $K$ inses vara den nxn tridiagonala matrisen med 2 på diagonalen och -1 vid sidan av diagonalerna, ty det avståndet $q_i$ som partikel i flytas bidrar både till att öka kraften på partikeln från fjädern i negativ riktning samt minska kraften i från fjädern i positiv riktning, varpå vid får dubbelt bidrag från $q_i$ och motriktade enkla bidrag från $q_{i-1}$ respektive $q_{i+1}$ från intilligande parkiklar. \\
Låt $u_i, i=1,...,n$ vara de ortonormala egenvektorerna till K och $\lambda_i, i=1,...,n$ de motsvarande egenvärdena. Detta ger oss enligt ovan egenfrekvenserna:
$\omega_i=\sqrt{\lambda_i} \omega_o. i=1,...,n$. Med den allmäna fysikaliska lösningen:
\begin{IEEEeqnarray*}{rCl}
 Q(t) &=&\sum c_i u_i \sin(\omega_i t+ \Phi_i)
\end{IEEEeqnarray*}
enligt samma beräkningar och resonemang som i uppgift 1.
\subsection{Deluppgift b}
För att illustrera egenfrekvenserna plottar vi de olika egensvängingarna över tid. %referens till bild
För att presentera egensvängingarna som stående vågor så betraktar vi för de olika egensvängingarna partiklarnas amplituder som fås ur egensvängingens egenvektor.  %referens till bild
