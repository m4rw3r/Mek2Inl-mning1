...
\section{Sats om matrisbeskrivning av ett system av $N$ partiklar}
	...
	\subsubsection*{$N = 3$:}
		...
		\begin{equation*}
			\left\{ \,
			\begin{IEEEeqnarraybox}[\IEEEeqnarraystrutmode][c]{rCl}
				\ddot{q_1} & = & -\frac{k q_1}{m} + \frac{k (q_2 - q_1)}{m} \\
				\ddot{q_2} & = & -\frac{k (q_2 - q_1)}{m} + \frac{k (q_3 - q_2)}{m} \\
				\ddot{q_3} & = & -\frac{k q_3}{m} + \frac{k (q_3 - q_2)}{m}
			\end{IEEEeqnarraybox}
			\right.
			\hspace{12pt}
			\Leftrightarrow
	...
	\subsubsection*{$N = \{x: x \in \N \land x > 3\}$:}
