...
%det finns problem  i hur vi definerar omega_o=sqrt(k/m), borde vara olika för olika massorna. BEHÖVER GÖRAS OM!
	...
	\begin{IEEEeqnarray*}{C}
		(2 \mu + 2) \frac{\omega^2}{\omega_o^2}^2- (2 \mu + 1) \frac{\omega^2}{\omega_o^2} - \frac{\omega^2}{\omega_o^2}^3 = 0 \\
		\Rightarrow
		\begin{cases}
			\frac{\omega_1^2}{\omega_o^2} = 0 \\
			\frac{\omega_2^2}{\omega_o^2} = 1 \\
			\frac{\omega_3^2}{\omega_o^2} = 1 + 2 \mu
		\end{cases}
		\Leftrightarrow
		\hspace{12pt}
		\begin{cases}
	...
	\begin{IEEEeqnarray*}{rCl}
		Q(t) &=& \frac{c_1}{\sqrt{3}}
			...
			&+& \frac{c_3}{\sqrt{2 + 4 \mu^2}}\begin{bmatrix}
				1 \\
				-2 \mu \\
				1
			\end{bmatrix}\sin\big(\sqrt{1 + 2 \mu}\omega_o t + \Phi_3 \big)
	\end{IEEEeqnarray*}
...
\section{Uppgift 4}
	...
	\begin{appendix}
		...
		\lstset{language=MATLAB, commentstyle=\textit, showspaces=false, showstringspaces=false, showtabs=false, tabsize=2, basicstyle=\scriptsize, breaklines, breakatwhitespace, postbreak=\ding{229}, extendedchars=true}
